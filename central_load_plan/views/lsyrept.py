...
def dump():
    """
    Dump the LSY database tables for development use.
    """
    manifest = {}
...
@lsyrept_bp.cli.command('from_remarks')
@click.argument('airline_iata_code')
@click.option('--limit', type=click.INT, default=10)
def from_remarks(airline_iata_code, limit):
    flights_with_crew_from_remarks = (
        db.select(
            ItemDaily.uno,
            ItemDaily.day_of_origin,
            ItemDaily.airline,
            ItemDaily.flight_no,
            ItemDaily.airport_c_is_dep,
            ItemDaily.airport_c_is_dest,
            RemarkOfEvent.remark,
        )
        .join(
            ItemDaily, ItemDaily.uno == RemarkOfEvent.uno,
        )
        .where(
            RemarkOfEvent.is_jumpseat,
            ItemDaily.day_of_origin <= date.today(),
        )
        .order_by(
            ItemDaily.day_of_origin.desc(),
        )
        .limit(limit)
    )

    airline_engine = get_lsyrept_engine(airline_iata_code)
    with Session(airline_engine) as session:
        for row in session.execute(flights_with_crew_from_remarks).mappings():
            print(row)
